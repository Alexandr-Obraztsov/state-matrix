...
TOOLS = [
    ...
    tool("sm_cases",
         "База знаний. Без kind — список видов параметров. С kind — пошаговая "
         "памятка по виду: слова-сигналы, по которым вид узнаётся в спеке; где искать "
         "его значения; шаги получения значений; корнер-кейсы по группам с объяснением, "
         "чем опасен каждый. Самые проработанные виды: число, деньги, строка, дата, "
         "запрос. Пройди шаги по порядку и проверь каждый кейс по спеке: нашёл — в "
    ...
    tool("sm_param_add",
         ...
         {"model": MODEL, "name": S("имя параметра"),
          ...
          "all_values": SL("ВСЕ значения из спеки, до объединения. Перед этим "
                           "загляни в sm_cases по виду параметра — там корнер-кейсы, "
