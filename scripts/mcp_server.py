...
REF = S("откуда это: раздел спеки (docs/spec.md:§3), цитата, ссылка на Confluence "
        "или Jira, номер задачи — любой текст. Если сослался на файл репозитория, "
        "место в нём проверяется: выдуманный раздел не пройдёт")
...
TOOLS = [
    ...
    tool("sm_cases",
         "База знаний — ОБЯЗАТЕЛЬНЫЙ шаг перед каждым sm_param_add. Без kind — "
         "список из 14 видов с синонимами. С kind — пошаговая памятка по виду: слова-сигналы, по которым вид узнаётся в спеке; где искать "
         "его значения; шаги получения значений; корнер-кейсы по группам с объяснением, "
         "чем опасен каждый. Пройди шаги по порядку и проверь каждый кейс по спеке: нашёл — в "
         "all_values, не нашёл, но важен — спроси пользователя.",
         {"model": MODEL, "kind": S("вид параметра или его синоним: строка, инпут, "
                                    "число, деньги, запрос, права, файл")},
         ["model"], sm.cmd_cases, {"root": ".", "kind": None}),

    tool("sm_learn",
         "Дописать корнер-кейс в базу знаний проекта. Вызывай, когда пользователь "
         "назвал случай, которого в базе не было, или когда нашёл в спеке "
         "повторяющийся подвох. Запись ложится в проект и переживает обновление "
         "плагина — в следующий раз подскажется сама.",
         {"model": MODEL, "kind": S("вид параметра, например «строка» или «промокод»"),
          "case": S("сам корнер-кейс, коротко"),
          "why": S("почему важен — что ломается или чем отличается"),
          "aka": SL("синонимы вида параметра")},
         ["model", "kind", "case"], sm.cmd_learn, {"root": ".", "why": None, "aka": None}),

    tool("sm_param_add",
         "Шаг 2. Входной параметр целиком. ПЕРЕД ВЫЗОВОМ обязательно вызови sm_cases "
         "с видом параметра и пройди памятку: сигналы, места поиска, шаги, корнер-кейсы. "
         "all_values — ВСЕ значения из спеки, "
         ...
         {"model": MODEL, "name": S("имя параметра"),
          ...
          "all_values": SL("ВСЕ значения из спеки, до объединения, включая "
                           "корнер-кейсы, найденные по памятке sm_cases"),
