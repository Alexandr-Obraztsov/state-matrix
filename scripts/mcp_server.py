...
TOOLS = [
    tool("sm_init",
         "Шаг 1. Завести модель по спецификации. Дальше прочитай спеку и ВСЁ, "
         "на что она ссылается — соседние документы, схемы, задачи в трекере — "
         "прежде чем объявлять параметры.",
         {"model": MODEL, "system": S("имя системы"), "source": S("путь к спецификации")},
         ["model", "system", "source"], sm.cmd_init, {"force": False, "root": "."}),

    tool("sm_param_add",
         "Шаг 2. Объявить входной параметр: имя, тип, описание, ссылка. Значения "
         "задаются позже. Объяви СРАЗУ ВСЕ параметры, какие нашёл в спеке.",
         {"model": MODEL, "name": S("имя параметра"),
          "type": S("тип: string, number, enum, boolean, endpoint, array, date, file"),
          "desc": S("что это за вход — одной фразой"), "from": REF, "root": ROOT},
    ...
    tool("sm_params",
         "Перечень объявленных параметров. Пользователь НЕ ВИДИТ вывод тулов: "
         "вставь его в чат целиком, и только потом спрашивай «все ли это параметры?».",
         {"model": MODEL}, ["model"], sm.cmd_params, {"root": "."}),

    tool("sm_param_values",
         "Шаг 3. Значения одного параметра. Сначала all_values — ВСЕ значения из "
         "спеки, включая корнер-кейсы. Потом values — классы после объединения, "
         "и grouping — почему объединённые дают один исход.",
         {"model": MODEL, "name": S("имя параметра"),
          ...
          "special": SL("значения вне матрицы, проверяются по одному: "
                        "таймаут, ответ не по схеме, неизвестный статус")},
         ["model", "name", "all_values"], sm.cmd_param_values, {"root": "."}),
    ...
    tool("sm_state_add",
         "Шаг 4. Состояние интерфейса. Покрой ВСЕ варианты UI: элемент показан и "
         "скрыт, загрузка, пусто, ошибка, заблокировано, свёрнуто, данные неполные. "
         "Порядок значим — частные случаи раньше общих, состояние без условия "
         "только последним.",
    ...
    tool("sm_state_rm", "Убрать состояние.", {"model": MODEL, "name": S("название")},
         ["model", "name"], sm.cmd_state_rm, {"root": "."}),

    tool("sm_states",
         "Состояния по порядку. Пользователь НЕ ВИДИТ вывод тулов — вставь его "
         "в чат целиком перед вопросом о полноте вариантов интерфейса.",
         {"model": MODEL}, ["model"], sm.cmd_states, {"root": "."}),

    tool("sm_rule_add",
         "Шаг 5. Правило свёртки матрицы. forbid вычёркивает невозможные комбинации, "
         "when+irrelevant схлопывает незначащие колонки. Основание берётся ТОЛЬКО "
         "из спеки: ссылка проверяется, выдуманная не пройдёт.",
    ...
    tool("sm_show",
         "Вся модель: параметры со значениями и основаниями объединений, правила, "
         "состояния. Пользователь НЕ ВИДИТ вывод тулов — вставь его в чат целиком "
         "перед вопросом о верности значений.",
         {"model": MODEL}, ["model"], sm.cmd_show, {"root": "."}),

    tool("sm_build",
         "Шаг 6. Собрать матрицу: проверка ссылок, инварианты, свёртка. Возвращает "
         "числа, состояния с числом строк, саму матрицу, спецзначения и находки. "
         "Пользователь НЕ ВИДИТ вывод тулов — вставь его в чат целиком, не сокращая "
         "таблицу. Откажет, если у параметров нет значений или нет состояний.",
         {"model": MODEL, "root": ROOT}, ["model"], sm.cmd_build, {"root": "."}),

    tool("sm_rows",
         "Строки матрицы. С no_state — только те, которым не досталось состояния: "
         "по каждой реши, описать состояние или запретить комбинацию.",
         ...
         ["result"], sm.cmd_rows, {"limit": 50, "no_state": False, "root": "."}),
]
