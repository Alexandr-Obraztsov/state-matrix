...
TOOLS = [
    ...
    tool("sm_cases",
         "База знаний по корнер-кейсам. Без kind — список видов параметров "
         "(строка, число, деньги, запрос, права, файл…). С kind — все известные "
         "корнер-кейсы этого вида и почему каждый важен. Обращаться необязательно, "
         "но полезно: здесь то, что чаще всего забывают при перечислении значений. "
         "Это подсказки, а не обязательный список — бери то, что описано в спеке, "
         "а про недостающее спрашивай пользователя.",
