...
TOOLS = [
    ...
    tool("sm_build",
         "Шаг 5. Собрать матрицу: проверка ссылок, инварианты, свёртка. Возвращает "
         "числа, состояния с числом строк, саму матрицу с номерами строк, "
         "спецзначения и находки. Пользователь НЕ ВИДИТ вывод тулов — вставь его "
         "в чат целиком, не сокращая таблицу.",
         {"model": MODEL, "root": ROOT}, ["model"], sm.cmd_build, {"root": "."}),

    tool("sm_assign",
         "Шаг 6. Привязать состояние к строкам матрицы по их номерам из последней "
         "сборки (r000, r001…). Строки без состояния — непокрытые случаи: по каждой "
         "либо назначь состояние, либо запрети комбинацию правилом, либо скажи "
         "пользователю, что это дыра в спеке. После привязок пересобери матрицу.",
