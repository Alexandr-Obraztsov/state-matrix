...
**Корзина по умолчанию пуста.** Типы заводятся по ходу разбора первой же спеки:
вопросы должны отражать твою предметную область, а не общие догадки. Пример
наполненной корзины из 19 типов — в `examples/catalog.json`.
...
- `param add` отказывает, пока не запрошен тип параметра из корзины
